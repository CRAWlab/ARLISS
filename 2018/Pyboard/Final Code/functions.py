...
def motor_accel(speed, time):
    for x in range(20, speed+1):
            motor_A.start((x*.97), 'cw')
            motor_B.start(x, 'ccw')
            pyb.delay(time)

def motor_deccel(speed, time):
    for x in range(1,speed-20):
        motor_A.start((speed-x)*.97, 'cw')
        motor_B.start(speed-x, 'ccw')
        pyb.delay(time)
    motor_A.start(0, 'cw')
    motor_B.start(0, 'ccw')
...
def course_correct(degree_to_turn):

    initial_read = razor_imu.get_one_frame()  #### HAVING TROUBLE GETTING YAW VALUE FROM IMU #######  #get current angle of device
    current_angle = initial_read[0]               #only yaw

    desired_angle = current_angle + degrees_to_turn     #add (or subtract) passed parameter of degrees to turn to current angle to determine desired angle

    while abs(desired_angle - current_angle) > 3:      #while the difference between the current angle and the desired is greater than a tolerance of 3...

        current_angle = razor_imu.get_one_frame()[0]       #updated current angle

        if current_angle > desired_angle:           #current angle is ccw of the desired angle, rotate to the right
            motor_A.start(20, 'cw')
            motor_B.start(20, 'cw')
            pyb.delay(100)
        elif current_angle < desiried_angle:        #current angle is cw of the desired andlge, rotate to the left
            motor_A.start(20, 'ccw')
            motor_B.start(20, 'ccw')
            pyb.delay(100)

    motor_A.start(0, 'cw')          #robot has entered into the tolerance band, loop has kicked out, Stop the motors.
    motor_B.start(0, 'cw')


def crude_course_correct(degree_to_turn):

    if degree_to_turn < -15:
    ...
    if degree_to_turn > 15:
        ...
        motor_A.start(25, 'ccw')
        motor_B.start(25, 'ccw')
        pyb.delay(500)
        motor_A.start(0, 'ccw')
        motor_B.start(0, 'ccw')
...
def spinout():
    motor_A.start(100, 'cw')
    motor_B.start(100, 'cw')
    pyb.delay(600)
    motor_A.stop()
    motor_B.stop()
...
def imu_pid2(duration, maxspeed):       #Matt's IMU control

    run_time = pyb.millis()
    initial_read = (None, None, None)

    # Try to read the IMU. If no data is received after 5000ms timeout
    while initial_read[0] is None and pyb.elapsed_millis(run_time) < 5000:
        initial_read = razor_imu.get_latest_angle_data()

    if initial_read[0] is not None:
        current_angle = initial_read[0]
        desired_angle = initial_read[0] # Only interested in yaw
        pyb.delay(2000)

        while pyb.elapsed_millis(run_time) < duration:

            current_gryo_angles = razor_imu.get_latest_angle_data()  ######## Having Difficulties getting yaw values from IMU#############

            if current_gryo_angles[0] is not None:
                current_angle = current_gryo_angles[0]
            else:
            ...
            if abs(angle_difference) > 3:
                ...
                if current_angle > desired_angle:       #heading ccw from desired angle
                    if abs(angle_difference) > maxspeed/2:         #limit motor speed difference in this condition
                        motor_A.start(maxspeed/2, 'cw')
                        motor_B.start(maxspeed, 'ccw')
                    else:
                        motor_A.start(maxspeed - 2*abs(angle_difference), 'cw')       #otherwise slow down motor as a function of angle difference
                        motor_B.start(maxspeed, 'ccw')

                if current_angle < desired_angle:       #heading cw from desired angle
                    if abs(angle_difference) > maxspeed/2:          #limit motor difference in this condition
                        motor_B.start(maxspeed/2, 'ccw')
                        motor_A.start(maxspeed, 'cw')
                    else:
                        motor_B.start(maxspeed - 2*abs(angle_difference), 'ccw')
                        motor_A.start(maxspeed, 'cw')
        motor_deccel(maxspeed, 50)
