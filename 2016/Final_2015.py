...
a predetermined destination in the ARLISS compeition. The code will operate as follows:
    1. Rover lands on ground, gets GPS coordinate.
    ...
    regression.
    (linear regression-- correlates the time the wheel is rotating with rover
    direction)
...
A = Left wheel
B = Right wheel
All pins must be connected on the Sparkfun breakout (all the VCC's and grounds)
GPS TX = 'Y2'
GPS RX = 'Y1'
XBEE DOUT (TX) = 'X10'
XBEE DIN (RX) = 'X9'
